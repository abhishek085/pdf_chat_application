...
# input_prompt = 'difference between coffee and latte'
# generated_text = base_model(input_prompt, max_length=512, do_sample=True)[0]['generated_text']
...
@st.cache_resource
def llm_pipeline():
    base_model = pipeline('text2text-generation', model = checkpoint
                 ,max_length = 256,
                 do_sample=True,
                 temperature = 0.8,
                 top_p=0.95,
...
def process_answer(instruction):
    ...
    combine_docs_chain = qa_llm(instruction)
    # qa = qa_llm()
...
def main():
    ...
    st.markdown("<h3 style='text-align: center; color: grey;'>Built by <a href='https://github.com/abhishek085'> Abhishek Rai</a></h3>", unsafe_allow_html=True)
